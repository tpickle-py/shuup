...
class AdminFieldRenderer(FieldRenderer):
    def __init__(self, field, **kwargs):
        ...
        self.show_help_block = bool(
            kwargs.pop("show_help_block", default_show_help_block)
        )

        kwargs["required_css_class"] = "required-field"
        kwargs["set_required"] = False
        kwargs["bound_css_class"] = (
            " "  # This is a hack, but Django-Bootstrap is silly and requires a truthy value.
        )
    ...
    def add_help_attrs(self, *args, **kwargs):
        ...
        if (
            not self.show_help_block
        ):  # Remove field help to avoid rendering `help-block`
            self.field_help = ""

    def append_to_field(self, html):
        from django.template import loader as template_loader

        edit_button = template_loader.render_to_string(
            "shuup/admin/forms/widgets/edit_button.jinja", {"field": self}
        )
        help_text = template_loader.render_to_string(
            "shuup/admin/forms/widgets/help_text.jinja", {"field": self}
        )
