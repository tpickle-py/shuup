...
def _load_provide_objects(category):
    ...
    if set(provide_specs) != set(loaded_provides.keys()):  # Changes occurred, reload provides
        _uncache(category)
        explanation = f"Loading provides {category}"
...
def _get_settings_value(setting_name):
    if not hasattr(settings, setting_name):
        raise ImproperlyConfigured(f"Error! The setting `{setting_name}` MUST be set.")
...
def _load_module(provide_category, setting_name, setting_value):
    ...
    if not object:
        raise ImproperlyConfigured(
            f"Error! Setting `{setting_name}` refers to a provide with identifier `{setting_value!r}`, but "
            f"it isn't one of the known identifiers in the `{provide_category}` category: `{sorted(get_identifier_to_object_map(provide_category).keys())!r}`."
        )
