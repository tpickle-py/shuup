...
def get_subscription_options_for_product(
    shop, product, supplier=None, user=None, **kwargs
):
...
def _get_cross_sell_products(
    ...
    if use_variation_parents:
        related_product_cross_sells = set(
            ...
            .values_list(
                Coalesce("product2__variation_parent_id", "product2_id"), "weight"
            )
            .distinct()
        )
    else:
        related_product_cross_sells = set(
            related_product_cross_sells.order_by("-weight")
            .values_list("product2_id", "weight")
            .distinct()
        )
    ...
    products = (
        catalog.get_products_queryset().filter(pk__in=products_ids).distinct()[:count]
    )
    return sorted(products, key=lambda product: products_ids.index(product.id))


@contextfunction
def get_products_bought_with(
    context, product: Product, count=5, orderable_only=True, use_variation_parents=True
):
    types = [ProductCrossSellType.BOUGHT_WITH, ProductCrossSellType.COMPUTED]
    return _get_cross_sell_products(
        context, product, types, count, orderable_only, use_variation_parents
    )
