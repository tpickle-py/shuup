...
class ContactGroupPriceDisplay(models.Model):
    ...
    hide_prices = models.BooleanField(default=None, null=True, blank=True, verbose_name=_("hide prices"))
...
class ContactGroup(TranslatableShuupModel):
    ...
    def __str__(self):
        if hasattr(self, "name") and self.name:
            return str(self.name)  # Convert lazy translations to string
...
class AnonymousContact(Contact):
    ...
    def groups(self):
        ...
        """
        _ = self.default_group  # Make sure group exists
