...
def bump_cache_for_shop_product(instance, shop=None):
    ...
    product_ids_to_bump = Product.objects.filter(
        ...
        | Q(id__in={value for pair_of_values in package_product_ids for value in pair_of_values})
...
def bump_cache_for_pk(cls, pk):
    ...
    """
    cache.bump_version(f"{_get_namespace_prefix(cls)}-{pk}")
...
def get_cache_key_for_context(identifier, item, context, **kwargs):
    ...
    return f"{namespace}:{identifier}_{key_hash}"
...
def _get_namespace_for_item(item):
    return f"{_get_namespace_prefix(item)}-{_get_item_id(item)}"
