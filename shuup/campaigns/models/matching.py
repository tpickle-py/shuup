...
def get_matching_context_conditions(context):
    ...
    ctx_cache_elements = {"customer": context.customer.pk or 0, "shop": context.shop.pk}
...
def update_matching_category_filters(shop_product, ids):
    ...
    CatalogFilterCachedShopProduct.objects.filter(
        filter__in=filters, shop_product__id__in=all_ids
    ).delete()
        ...
        for sp in all_shop_products:
            if filter.matches(sp):
                CatalogFilterCachedShopProduct.objects.create(
                    filter=filter, shop_product=sp
                )
                context_cache.bump_cache_for_shop_product(
                    sp
                )  # is this necessary, smells like double


def update_matching_catalog_filters(shop_product_or_filter):
    if isinstance(shop_product_or_filter, CatalogFilter):
        CatalogFilterCachedShopProduct.objects.filter(
            filter=shop_product_or_filter
        ).delete()
    ...
    CatalogFilterCachedShopProduct.objects.filter(
        filter__id__in=ids, shop_product__id__in=all_ids
    ).delete()
...
def get_matching_catalog_filters(shop_product):
    return shop_product.cached_catalog_campaign_filters.values_list(
        "filter__id", flat=True
    )
