...
def test_class_refunded():
    ...
    Script.objects.create(
        shop=shop, event_identifier="order_status_changed", name="Script", enabled=True
    )
    ...
    with mock.patch(
        "shuup.front.notify_events.OrderStatusChanged", new_callable=get_mocked_cls
    ) as mocked:
        order.change_status(
            next_status=OrderStatus.objects.get_default_processing(), user=customer.user
        )
        mocked.assert_called()
        order.refresh_from_db()
        assert mocked.call_args[1]["order"] == order
        assert (
            mocked.call_args[1]["old_status"]
            == OrderStatus.objects.get_default_initial()
        )
        assert (
            mocked.call_args[1]["new_status"]
...
def test_shipment_events():
    ...
    with mock.patch(
        "shuup.front.notify_events.ShipmentCreated", new_callable=get_created_mocked_cls
    ) as mocked1:
        shipment = order.create_shipment({product: 1}, supplier=supplier)
        mocked1.assert_called()

    with mock.patch(
        "shuup.front.notify_events.ShipmentSent", new_callable=get_sent_mocked_cls
    ) as mocked2:
        shipment.set_sent()
        mocked2.assert_called()

    with mock.patch(
        "shuup.front.notify_events.ShipmentDeleted", new_callable=get_soft_mocked_cls
    ) as mocked3:
