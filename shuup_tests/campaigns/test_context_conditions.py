...
def create_random_product_and_campaign(
    shop, conditions, original_price_value, discount_value
):
    campaign = CatalogCampaign.objects.create(shop=shop, name="test", active=True)
    ProductDiscountAmount.objects.create(
        discount_amount=discount_value, campaign=campaign
    )
    ...
    product = create_product(
        "Some crazy product", shop, default_price=original_price_value
    )
    return product
...
def test_context_contact_group_condition():
    ...
    product = create_random_product_and_campaign(
        request.shop, [condition], original_price_value, discount_value
    )

    discounted_value = original_price_value - discount_value
    assert_product_price_value_with_customer(
        request, customer, product, discounted_value
    )

    request.customer.groups.clear()
    assert_product_price_value_with_customer(
        request, customer, product, original_price_value
    )
...
def test_group_condition_with_anonymous_contact():
    ...
    discounted_value = original_price_value - discount_value
    assert_product_price_value_with_customer(
        request, request.customer, product, discounted_value
    )
...
def test_context_contact_condition():
    ...
    condition.save()
    assert_product_price_value_with_customer(
        request, random_person, product, original_price_value
    )
