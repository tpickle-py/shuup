...
def test_product_type_filter(rf):
    ...
    assert (
        product_type_filter.values.count() == 1
    )  # both have same product type so only 1 value here
...
def test_product_filters_with_variation_parents(rf):
    request, shop, group = initialize_test(rf, False)
    product = create_product(
        "test", shop, default_price=20, mode=ProductMode.SIMPLE_VARIATION_PARENT
    )
    child_products = []
    for x in range(0, 3):
