...
def change_addresses(live_server, browser, order):
    ...
    wait_until_condition(
        browser, condition=lambda x: x.is_text_present(edit_address_title)
    )
    # Do nothing just hit the save
    click_element(browser, "button[form='edit-addresses']")
    assert (
        order.log_entries.filter(identifier=ADDRESS_EDITED_LOG_IDENTIFIER).count()
        == order_edited_log_entries
    )
...
def set_status(browser, order, status):
    ...
    click_element(
        browser, "button[formaction='%s'][value='%s']" % (form_action, status.pk)
    )
...
def check_log_entries_count(browser, order, target_count):
    wait_until_condition(
        browser,
        condition=lambda x: order.log_entries.filter(
            identifier=ADDRESS_EDITED_LOG_IDENTIFIER
        ).count()
        == target_count,
    )
