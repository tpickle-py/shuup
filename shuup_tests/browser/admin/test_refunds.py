...
def _check_create_refund_link(browser, order, present):
    ...
    wait_until_condition(
        browser, lambda x: x.is_element_present_by_css("a[href='%s']" % url) == present
    )
...
def _test_create_full_refund(browser, live_server, order):
    ...
    wait_until_condition(
        browser,
        lambda x: x.is_text_present(
            "Refunded: %s" % format_money(order.shop.create_price("0.00"))
        ),
    )
    wait_until_condition(
        browser,
        lambda x: x.is_text_present(
            "Remaining: %s" % format_money(order.taxful_total_price)
        ),
    )
...
def _test_refund_view(browser, live_server, order):
    ...
    assert (
        len(browser.find_by_css("#id_form-0-line_number option")) == 12
    )  # blank + arbitrary amount + num lines
    ...
    assert decimal.Decimal(
        browser.find_by_css("#id_form-1-amount").first.value
    ) == decimal.Decimal("100.00")
    assert (
        int(decimal.Decimal(browser.find_by_css("#id_form-1-quantity").first.value))
        == 10
    )
