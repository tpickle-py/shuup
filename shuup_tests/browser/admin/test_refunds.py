...
def _test_create_full_refund(browser, live_server, order):
    ...
    wait_until_condition(
        browser,
        lambda x: x.is_text_present("Refunded: %s" % format_money(order.shop.create_price("0.00"))),
    )
    wait_until_condition(
        browser,
        lambda x: x.is_text_present("Remaining: %s" % format_money(order.taxful_total_price)),
    )
    url = reverse("shuup_admin:order.create-full-refund", kwargs={"pk": order.pk})
    click_element(browser, "a[href='%s']" % url)
    wait_until_condition(
        browser,
        lambda x: x.is_text_present("Refund Amount: %s" % format_money(order.taxful_total_price)),
    )
