...
def test_carousel_multi_slide(browser, admin_user, live_server, settings):
    ...
    for slide_id in [0, 1, 2]:
        ...
        wait_until_appeared(
            browser,
            ".select2-container #select2-id_slides-%d-category_link-results li" % slide_id,
        )
        click_element(
            browser,
            ".select2-container #select2-id_slides-%d-category_link-results li:last-child" % slide_id,
        )
