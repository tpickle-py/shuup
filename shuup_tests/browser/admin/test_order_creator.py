...
def test_order_creator_view_1(browser, admin_user, live_server, settings):
    ...
    object_created.connect(
        _add_custom_order_created_message,
        sender=Order,
...
def test_order_creator_view_2(browser, admin_user, live_server, settings):
    ...
    assert (
        Order.objects.first()
        .log_entries.filter(identifier=OBJECT_CREATED_LOG_IDENTIFIER)
        .count()
        == 1
    )
...
def _test_regions(browser, person):
    ...
    move_to_element(
        browser, "input[name='billing-region']"
    )  # To ensure all inputs required test is available
...
def _test_quick_add_lines(browser):
    ...
    wait_until_condition(
        browser,
        lambda x: len(browser.find_by_css("#quick-add .select2-container--open")) == 1,
    )
    ...
    assert len(browser.find_by_css("#quick-add .select2-container--open")) == 1, (
        "select is open after add"
    )
    assert line_items.first.find_by_css('input[name="quantity"]').first.value == "1", (
        "one piece added"
    )
    ...
    wait_until_condition(
        browser,
        lambda x: x.find_by_css('#lines input[name="quantity"]').first.value == "2",
    )
    line_items = browser.find_by_id("lines").find_by_css(".list-group-item")
    assert len(line_items) == 1, "only one line item exists"
    assert line_items.first.find_by_css('input[name="quantity"]').first.value == "2", (
        "two pieces added"
    )
    ...
    wait_until_condition(
        browser,
        lambda x: x.find_by_css('#lines input[name="quantity"]').first.value == "3",
    )
    assert line_items.first.find_by_css('input[name="quantity"]').first.value == "3", (
        "three pieces added"
    )
...
def _test_customer_data(browser, person):
    ...
    assert (
        browser.find_by_css("input[name='ship-to-billing-address']").first.checked
        == False
    )
    ...
    wait_until_condition(
        browser,
        lambda x: x.find_by_css("input[name='billing-tax_number']").first["required"],
    )
    assert len(browser.find_by_css("input[name='shipping-name']")) == 0, (
        "shipping address column is hidden"
    )
    ...
    assert (
        browser.find_by_name("billing-street").value
        == person.default_billing_address.street
    )
    assert (
        browser.find_by_name("billing-city").value
        == person.default_billing_address.city
    )
    assert (
        browser.find_by_name("billing-country").value
        == person.default_billing_address.country
    )
...
def _test_add_lines(browser):
    ...
    wait_until_condition(
        browser,
        lambda x: len(x.find_by_css("#lines .list-group-item"))
    ...
    wait_until_condition(
        browser,
        lambda x: x.is_element_present_by_css("#lines .list-group-item:last-child a"),
    )
    ...
    wait_until_condition(
        browser,
        lambda x: x.find_by_css(
            '#lines .list-group-item:last-child input[name="total"]'
        ).first.value
        == "10",
        timeout=100,
    )
    last_line_item = browser.find_by_css("#lines .list-group-item:last-child")
    assert last_line_item.find_by_css('input[name="quantity"]').first.value == "1", (
        "1 piece added"
    )
    assert last_line_item.find_by_css('input[name="total"]').first.value == "10", (
        "line item total is 10"
    )
...
def _test_confirm(browser):
    ...
    assert str(total) in browser.find_by_css(".order-footer h2").text, (
    ...
    assert len(browser.find_by_css("table tbody tr")) == 4, (
