...
def test_bar_chart():
    ...
    locale = "pt_br"
    chart = BarChart(
        "ma biultiful xart", labels, data_type=ChartDataType.NUMBER, locale=locale
    )
    ...
    dataset1 = OrderedDict(
        {"type": ChartType.BAR, "label": "some bars #1", "data": [1, 2, 3]}
    )
    dataset2 = OrderedDict(
        {"type": ChartType.BAR, "label": "some bars #2", "data": [2, 3, 4]}
    )
    ...
    for i in range(len(chart_config["data"]["datasets"])):
        for j in range(len(chart_config["data"]["datasets"][i]["data"])):
            assert (
                chart_config["data"]["datasets"][i]["data"][j] == datasets[i]["data"][j]
            )

            formatted_data = chart_config["data"]["datasets"][i]["formatted_data"][j]
            assert formatted_data == format_decimal(
                datasets[i]["data"][j], locale=locale
            )
...
def test_bar_chart_percent():
    ...
    locale = "pt_br"
    chart = BarChart(
        "ma biultiful xart %", labels, data_type=ChartDataType.PERCENT, locale=locale
    )

    dataset1 = OrderedDict(
        {"type": ChartType.BAR, "label": "some bars #1", "data": [0.1, 0.2, 0.3]}
    )
    dataset2 = OrderedDict(
        {"type": ChartType.BAR, "label": "some bars #2", "data": [0.45, 0.55, 0.999]}
    )
    datasets = [dataset1, dataset2]
    ...
    for i in range(len(chart_config["data"]["datasets"])):
        for j in range(len(chart_config["data"]["datasets"][i]["data"])):
            ...
            formatted_data = chart_config["data"]["datasets"][i]["formatted_data"][j]
            assert formatted_data == format_percent(
                datasets[i]["data"][j], locale=locale
            )
...
def test_mixed_chart():
    ...
    dataset3 = OrderedDict(
        {"type": ChartType.LINE, "label": "some lines #1", "data": [5, 6, 7]}
    )
    dataset4 = OrderedDict(
        {"type": ChartType.LINE, "label": "some lines #2", "data": [8, 9, 10]}
    )
